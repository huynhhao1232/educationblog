...
from adminpage.admission_views import (  # noqa: E402,F401
    admission_vocational_manage,
    campus_shift_group_manage,
    delete_all_admissions,
    delete_cccd_excel,
    download_admission_docx,
    export_approved_admissions,
    export_conduct_template,
    get_Admission,
    get_Letter,
    get_district_name,
    get_province_name,
...
def _assign_exam_numbers_for_all_students(
    ...
    # ĐS/DS dùng số 9 (sau HT); cùng prefix thì sắp theo mã cơ sở.
    campus_order = [
        code
...
_EXAM_SORT_CAMPUS_RANK = {
    ...
    "CN": "6",
    "ĐS": "7",
    "DS": "7",  # phòng khi dữ liệu dùng DS không dấu # phòng khi dữ liệu dùng DS không dấu
    "HT": "8", # phòng khi dữ liệu dùng DS không dấu
}
...
def _fill_exam_giao_nhan_merged(
    ...
    *,
    ...
    combo_label_override: str | None = None,
    subjects_ws1_override: list[str] | None = None,
    phong_thi_header_override: str | None = None,
    th_label_fn=None,
) -> None:
    """
    ...
    **compact** — mẫu chỉ A–G: C = SBD, D–E = HỌ VÀ TÊN, F = Lớp, G = Ghi chú (không cột Phòng, không khối môn).
    ...
    combo_label = combo_label_override or _build_exam_export_combo_label(all_assignments)
    names: list[str] = []
    ...
    if subjects_ws1_override is not None:
        subjects_ws1 = list(subjects_ws1_override)
    else:
    ...
    def _row_th_label(stu, *, _room=None):
        if th_label_fn is not None:
            try:
                return th_label_fn(stu, room=_room)
            except TypeError:
                return th_label_fn(stu)
        return _exam_subject_group_th_label(stu)
    ...
    _set_cell_value(
        ws1, 4, 13,
        phong_thi_header_override or f'PHÒNG THI: {room_header}',
    )
        ...
        for idx, a in enumerate(assigns, start=1):
            ...
            if include_th_column:
                if th_kind == 'compact':
                    ...
                    ws1.cell(row=row_num, column=2, value=_row_th_label(stu, _room=room))

...
def get_provinces(request):
    try:
        print("Fetching provinces data from GitHub...")
        url = "https://raw.githubusercontent.com/kenzouno1/DiaGioiHanhChinhVN/master/data.json"
        response = requests.get(url)
        response.raise_for_status()  # Raise an exception for bad status codes
        data = response.json()
        print("Raw data from GitHub:", data[:2])  # Print first 2 items for debugging
        ...
        print("Transformed provinces data:", provinces[:2])  # Print first 2 items
        ...
        return JsonResponse({'error': f'Invalid JSON data: {str(e)}'}, status=500)
...
def get_districts(request, province_code):
    try:
        print(f"Fetching districts for province code: {province_code}")
        url = "https://raw.githubusercontent.com/kenzouno1/DiaGioiHanhChinhVN/master/data.json"
        response = requests.get(url)
        response.raise_for_status()
        data = response.json()

        # Find the province and get its districts
        province = next((p for p in data if p["Id"] == province_code), None)
        if province:
            districts = [{"code": d["Id"], "name": d["Name"]} for d in province["Districts"]]
            print(f"Found {len(districts)} districts for province {province_code}")
            return JsonResponse({'districts': districts})
        print(f"Province not found with code: {province_code}")
...
def get_wards(request, district_code):
    try:
        print(f"Fetching wards for district code: {district_code}")
        url = "https://raw.githubusercontent.com/kenzouno1/DiaGioiHanhChinhVN/master/data.json"
        response = requests.get(url)
        response.raise_for_status()
        data = response.json()

        # Find the district and get its wards
        for province in data:
            district = next((d for d in province["Districts"] if d["Id"] == district_code), None)
            if district:
                wards = [{"code": w["Id"], "name": w["Name"]} for w in district["Wards"]]
                print(f"Found {len(wards)} wards for district {district_code}")
                return JsonResponse({'wards': wards})
        print(f"District not found with code: {district_code}")
        return JsonResponse({'wards': []})
    except Exception as e:
        print(f"Error in get_wards: {str(e)}")
        return JsonResponse({'error': str(e)}, status=500)
